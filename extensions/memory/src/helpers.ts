...
export function mentionsCurrentPr(pr: PrRecord | undefined, text: string): boolean {
	...
	if (pr.prNumber != null && (text.includes(`#${pr.prNumber}`) || text.includes(`/pull/${pr.prNumber}`))) return true;
	return false;
}

export function getSessionTaskId(ctx: ExtensionContext, entryType: string): string | null | undefined {
