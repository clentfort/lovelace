...
export function parsePrContext(text: string): ParsedPrContext | undefined {
	...
	const prNumber = urlMatch ? Number(urlMatch[1]) : Number(text.match(/\b(?:PR\s*#?|#)(\d+)\b/i)?.[1] ?? NaN);
	const lines = text.split("\n").map((line) => line.trim()).filter(Boolean);
	const titleLine = lines.find((line) => /^title\s*:/i.test(line));
	let title = titleLine ? compactLine(titleLine.replace(/^title\s*:\s*/i, "")) : undefined;
	if (!title) {
		const naturalTitle = lines.find((line) => /pull request/i.test(line) && !line.includes("http") && line.length < 180);
